...
function verticalmenu_health()
{
    global $verticalmenu_memory_result;
    ...
    echo '<br />';
    echo '</strong>';
    $verticalmenu_memory_result = ob_get_contents();
    ob_end_clean();
    function verticalmenu_add_memory_test($tests)
    {
        $tests['direct']['memory_plugin'] = array(
            'label' => __('My Memory Test', 'verticalmenu'),
            'test' => 'verticalmenu_memory_test',
        );
        return $tests;
    }
    $perc = $memory['usage'] / $memory['wp_limit'];
    if ($perc > .7) {
        add_filter('site_status_tests', 'verticalmenu_add_memory_test');
    }
    function verticalmenu_memory_test()
    {
        global $verticalmenu_memory_result;
        $result = array(
            'badge' => array(
            ...
            'description' => $verticalmenu_memory_result . '  ' . sprintf(
                '<p>%s</p>',
