...
( function( api ) {
	var cssTemplate = wp.template( 'verticalmenu-color-scheme' ),
		colorSchemeKeys = [
			'background_color',
		...
		ready: function() {
			if ( 'color_scheme' === this.id ) {
				this.setting.bind( 'change', function( value ) {
					...
					api( 'menu_textcolor' ).set( colorScheme[value].colors[6] );
					api.control( 'menu_textcolor' ).container.find( '.color-picker-hex' )
